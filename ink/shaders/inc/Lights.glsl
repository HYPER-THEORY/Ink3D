...
vec3 apply_point_light(Material mat, Geometry geom, PointLight light) {
	/* check whether the light is visible */
	if (!light.visible) return vec3(0.);
	
	/* calculate the vector form world to light */
	vec3 light_vec = light.position - geom.position;
...
vec3 apply_spot_light(Material mat, Geometry geom, SpotLight light) {
	/* check whether the light is visible */
	if (!light.visible) return vec3(0.);
	...
	light_color *= light.cast_shadow ?
		shadowing(light.shadow, geom.position, geom.normal) : 1.;
...
vec3 apply_directional_light(Material mat, Geometry geom, DirectionalLight light) {
	/* check whether the light is visible */
	if (!light.visible) return vec3(0.);
...
vec3 apply_hemisphere_light(Material mat, Geometry geom, HemisphereLight light) {
	/* check whether the light is visible */
	if (!light.visible) return vec3(0.);

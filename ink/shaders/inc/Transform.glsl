...
/* Determines whether camera is perspective. */
bool is_perspective(mat4 proj) {
	return proj[2][3] == -1.;
}

/* Return the linearized depth with perspective camera. */
float linearize_depth_persp(float depth, float near, float far) {
	return 2. * near * far / (near + far + (depth * 2. - 1.) * (near - far));
}

/* Return the linearized depth with orthographic camera. */
...
/* Return the linearized depth with whatever camera. */

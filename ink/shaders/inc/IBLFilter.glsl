...
vec4 texture_env(samplerCube map, int max_lod, vec3 dir, float roughness) {
	float mip = 0.0;
	if (roughness >= 0.4) {
...
vec3 ibl_specular(samplerCube map, int max_lod, vec3 view_dir, vec3 normal, float roughness) {
...
vec3 ibl_diffuse(samplerCube map, int max_lod, vec3 normal) {
